...
from src.config import multiverse_params, epoch_windows, baseline_windows, translation_table, subjects

""" HEADER END """

# define multiverse parameter space
multiverse_params = {
        'ref': ['average'], # 'Cz' instead of FCz because some experiments have FCz as electrode of interest (would be 0 else)
        'hpf': [0.1], # was 0.01, but the raw data has implicit hpf of 0.03 already
        'lpf': [20], # 6 Hz for alpha exclusion, Bae and Luck 2018, 2019a, 2019b (Ref in Bae 2021)
        'emc': ['ica'],  # 'peak-to-peak', 'regression'
        'mus': [None], 
        'det': ['linear'], 
        'base': ['200ms'], # "None" is ommitted, makes no sense not to detrend
        'ar': ['int'], 
        }
